...
import 'package:flutter_simple_quizapp/network/exception.dart';
import 'package:flutter_simple_quizapp/utils/logger.dart';
import 'package:flutter_simple_quizapp/utils/secure_storage_utils.dart';
...
class ApiClient {
  ...
  Future<dynamic> post(
    ...
    try {
      final uri = _buildUrl(url, params: params);
      log.fine('[HTTP POST REQUEST: $uri, body: $jsonBody, headers: $updatedHeaders]');
  ...
  Future<dynamic> put(
    ...
    try {
      ...
      log.fine('[HTTP PUT RESPONSE: $uri, headers: $updatedHeaders, body: ${response.body}]');
      responseJson = _response(response);
    } on SocketException {
      throw FetchDataException('No Internet connection');
    }
